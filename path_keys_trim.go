...
to allow:
    ...
    $ vault write alicloudkms/keys/config/my-key \
        min_version=42

Then execute the trim call:

    $ vault write -f alicloudkms/keys/trim/my-key
...
// pathKeysTrimWrite corresponds to PUT/POST/DELETE alicloudkms/keys/trim/:key and
