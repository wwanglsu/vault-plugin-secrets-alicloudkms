...
```text
$ vault secrets enable alicloudkms
```

Optionally configure the backend with GCP credentials:

```text
$ vault write alicloudkms/config credentials="..."
```
...
```text
$ vault write alicloudkms/encrypt/my-key plaintext="hello world"
```

Decrypt the data:

```text
$ vault write alicloudkms/decrypt/my-key ciphertext="..."
```
...
1. Clone the repo:
    ...
    ```text
    $ git clone https://github.com/wwanglsu/vault-plugin-secrets-alicloudkms
    $ cd vault-plugin-secrets-alicloudkms
    ```
...
1. Copy the compiled binary into a scratch dir:
    ...
    ```text
    $ cp $(which vault-plugin-secrets-alicloudkms) ./bin/
    ```
