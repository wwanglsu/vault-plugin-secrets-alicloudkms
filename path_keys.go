...
func (b *backend) pathKeys() *framework.Path {
	fmt.Println("This is test on 4/16/2020-alicloudkms pathKeys()")
...
func (b *backend) pathKeysCRUD() *framework.Path {
	fmt.Println("This is test on 4/16/2020-alicloudkms pathKeysCRUD()")
	return &framework.Path{
...
it.
    ...
    $ vault write alicloudkms/keys/my-key \
...
current primary key version, read from the path:
    ...
    $ vault read alicloudkms/keys/my-key
...
Cloud KMS, and delete Vault's reference to the crypto key.
    ...
    $ vault delete alicloudkms/keys/my-key
...
// pathKeysRead corresponds to GET alicloudkms/keys/:name and is used to show
...
// pathKeysList corresponds to LIST alicloudkms/keys and is used to list all keys
// in the system.
...
// pathKeysWrite corresponds to PUT/POST alicloudkms/keys/create/:key and creates a
...
func (b *backend) pathKeysWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	fmt.Println("This is test on 4/16/2020-alicloudkms pathKeysWrite(), alicloudkms/keys/create/:key")
	/*kmsClient, closer, err := b.KMSClient(req.Storage)
	if err != nil {
		return nil, err
	}
...
// pathKeysDelete corresponds to PUT/POST alicloudkms/keys/delete/:key and deletes an
